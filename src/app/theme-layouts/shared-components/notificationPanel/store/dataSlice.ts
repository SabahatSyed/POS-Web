...
export const getNotifications = createAppAsyncThunk('notificationPanel/getData', async () => {
	...
	const data = (await response.data.records) as NotificationModelType[];
...
export const addNotification = createAppAsyncThunk(
	...
	async (item: NotificationModelType) => {
		const response = await axios.post(`/api/notifications/add`, { ...item });

		const data = (await response.data.record) as NotificationModelType;
