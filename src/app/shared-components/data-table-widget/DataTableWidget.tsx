							...
							{columns.map((column, index) => (
								...
								>
									<Typography
										...
										className="font-semibold text-white text-12 whitespace-nowrap text-center "
									>
										{column.title}
									</Typography>
								</TableCell>
						...
						{records.map((row, index) => (
							...
							>
									...
									switch (column.type) {
										...
										case 'date': {
											return (
												...
												>
													{value ? (
														<Typography className='text-center whitespace-nowrap'>{format(new Date(value), 'MMM dd, y')}</Typography>
													) : ''}
										...
										case 'datetime': {
											return (
												...
												>
													{value ? (
														<Typography className='text-center whitespace-nowrap'>{format(new Date(value), 'MMM dd, y hh:mm a')}</Typography>
													) : ''}													
												</TableCell>
											);
										}
										...
										default: {
											return (
												<TableCell
													...
													scope="row"
												>
													<Typography className='text-center whitespace-nowrap'>{value}</Typography>
												</TableCell>
											);
										}
									}
								...
								<TableCell className='w-[2px]'>
									<div
										className={`flex items-center w-50 justify-end ${selectedRow === index.toString() ? 'show-actions' : ''
											}`}
										...
										{props.rowActions.map((action, actionIndex) => (
											...
											>
												<Tooltip title={action.tooltip} placement="bottom">
													<IconButton aria-label="edit" onClick={(event: any) => {event.stopPropagation(); handleRowActionClick(action, row)}}>
