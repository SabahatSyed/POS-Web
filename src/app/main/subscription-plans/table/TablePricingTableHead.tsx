...
import { getInvoicesRecords } from '../../subscription/store/InvoiceSlice';
...
function TablePricingTableHead(props: TablePricingTableHeadProps) {
	...
	const { period = 'month', data, payment_method_id, sub_id, setSelectedPlan, selectedPlan} = props;
	const {
    id,
    description,
    feature_01,
    ...
    <Box className="flex flex-col " sx={{ backgroundColor: "background.paper" }}>
       <FormControlLabel value={data}
                    ...
                    <div className="flex gap-20 w-full">
                      <div>
                      <Radio
												checked={subscription_id === selectedPlan?.id}
												onChange={() => setSelectedPlan(data)}
												value="plan"
												name="radio-buttons"
												className={clsx(
                      ...
                      <div className="w-full ">
                        <div className='flex gap-20 items-center'>
                            <Typography className='text-xl font-600'>{name} - {period === "month" ?  "Monthly" : "Yearly" } </Typography>
                            {popular && (
                                <Chip
                                  label="POPULAR"
                                  color="secondary"
                        ...
                        <div className="flex justify-between w-full items-center">
                          <div>
                            <Typography className="text-lg" color="text.secondary">
                              USD {period === "month" && monthly_price}
                              {period === "year" && yearly_price} x {period === "month" && "1 Month"}
                              {period === "year" && "1 Year"}
                            </Typography>
                          </div>
                          <Typography className="text-lg" color="text.secondary">
