...
export const getContact = createAppAsyncThunk<ContactType, string>(
	...
	async (id, { rejectWithValue }) => {
		try {
			...
			const data = (await response.data.record) as ContactType;
...
export const addContact = createAppAsyncThunk<ContactType, ContactType>(
	...
	async (contact: ContactType, { rejectWithValue }) => {
		try {
			const response = await axios.post('/api/contacts', ContactModel(contact));
			return response.data.record as ContactType;
		} catch (error) {
			const axiosError = error as AxiosError;
			return rejectWithValue(axiosError.message);
		}
