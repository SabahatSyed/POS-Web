...
import { addRecord, getRecords, updateRecord } from '../store/agentInviteDataSlice';
 ...
 * AgentInviteFormPage
 */
function AgentInviteFormPage() {

	/**
	 * Form Validation Schema
	 */

	const title = 'Agent Invite';
	...
	const schema = yup.object().shape({
		...
		split_percentage: yup.string().required('You must enter a value'),
	});
	...
	const [rowData, setRowData] = useState<AgentInvite | undefined>(undefined);
	const [loading, setLoading] = useState(false);
	const { isValid, dirtyFields, errors, touchedFields } = formState;

	const onSubmit = async (formData: AgentInvite) => {
					...
					<Controller
						render={({ field }) => (
							<TextField
								...
								variant="outlined"
								className='bg-white'
								error={!!errors.split_percentage}
								helperText={errors?.split_percentage?.message}
								required
								fullWidth
							/>
						)}
...
export default AgentInviteFormPage;