...
import { getRecords, selectRecords, deleteRecord } from '../store/blockPurchaseFormulaDataSlice';
...
import { BlockPurchaseFormula } from '../types/dataTypes';


/**
 * The BlockPurchaseTablePage.
 */
function BlockPurchaseFormulaTablePage() {
	const navigate = useNavigate();

	const data = useAppSelector(selectRecords);

	const title = 'Block Purchase Formula';
	...
	function onTableEvent(event: TableEvent) {
		...
		if (event.action == 'create') {
			navigate(`/blockpurchaseformula/form`);
		}
		
		if (event.event == 'rowAction') {
			if (event.action == 'onEdit') {
				const row = event.params.row as BlockPurchaseFormula;
				navigate(`/blockpurchaseformula/form/${row._id}`);
...
export default BlockPurchaseFormulaTablePage;
